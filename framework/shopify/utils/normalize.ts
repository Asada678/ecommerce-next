...
const normalizeProductImages = ({ edges }: { edges: Array<ImageEdge> }) =>
  edges.map(({ node: { originalSrc: url, ...rest } }) => ({
    url:
      process.env.NEXT_PUBLIC_FRAMEWORK === 'shopify_local' ? `/image/${url}` : url ?? '/product-image-placeholder.svg',
    ...rest,
  }))

const normalizeProductPice = ({ currencyCode, amount }: MoneyV2) => {
  return {
    value: +amount,
    currencyCode,
  }
...
export function normalizeProduct(productNode: ShopifyProduct): Product {
  ...
  const product = {
    ...
    price: normalizeProductPice(priceRange.minVariantPrice),
