    ...
    <Container>
      <div className={cn(style.root, 'fit', 'mb-5')}>
          ...
          <ProductSlider>
            {product.images.map((image) => (
              <div className={style.imageContainer} key={image.url}>
                <Image className={style.img} src={image.url} alt={image.alt} width={1050} height={1050} quality="85" />
              </div>
            ))}
          </ProductSlider>
        </div>
        <div className={style.sidebar}>
          ...
          <div>
            <Button className={style.button} onClick={addToCart} isLoading={isLoading}>
              Add to Cart
            </Button>
          </div>
        </div>
